...
const AgendaCreate = () => {
    ...
    const [content, setContent] = useState([]); // [{no: "1", description: "..."}]
    const [participants, setParticipants] = useState([]); // Array of person objects
    ...
    const addMainPoint = () => {
        // Find next main number
        const mainPoints = content.filter(p => !String(p.no).includes('.'));
        const nextNo = (mainPoints.length + 1).toString();
        setContent([...content, { no: nextNo, description: "" }]);
    };

    const addSubPoint = (parentIndex) => {
        const parentNo = String(content[parentIndex].no);
        const subPoints = content.filter(p => String(p.no).startsWith(`${parentNo}.`));
        const newNo = `${parentNo}.${subPoints.length + 1}`;

        const newContent = [...content];
        // Insert after the last subpoint of this parent
    ...
    const handleCreate = async () => {
        if (!form.subject || !form.date) {
            toast.warn("Please fill in at least Subject and Date.");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/projectAgenda/add/${projectId}`, {
                method: "POST",
            ...
            toast.error(error.message || "Failed to create agenda.");
        } finally {
            setSaving(false);
        }
    };
            ...
            <main className="flex-1 p-8 bg-gray-900 overflow-y-auto text-gray-300">
                <header className="mb-8 flex justify-between items-center">
                    ...
                    >
                        ...
                        <span>{saving ? 'Creating...' : 'Create Agenda'}</span>
                    </button>
                </header>
                ...
                <div className="w-full space-y-6">
                    ...
                    <div className="bg-gray-800 border border-gray-700 p-6 rounded-lg shadow-md border-blue-500/50">
                        <div className="flex justify-between mb-4 border-b border-gray-700 pb-2">
                            <h3 className="text-lg font-bold text-white">New Agenda Details</h3>
                        </div>
                        ...
                        <table className="w-full text-left border-collapse text-sm">
                            <thead>
                                <tr className="bg-gray-900/40 text-gray-400">
                                    ...
                                    <th className="p-3 border-b border-gray-700 w-16">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
