...
function DprEditor() {
  ...
  const agencyDict = ['A TO Z GRILL FABRICATION', 'AAKASH UNIVERSAL', 'ABHISHEK SHIRKE', 'ADITYA ENTERPRISES', 'AETREUM CONCRETE', 'AP INTERIOR', 'AP INTERIORS', 'ART DECORATOR', 'ASTECH ENGINEERS', 'ATERUM CONCRETE', 'BANDU PRABHU NARWADE', 'BANNAPPA PUJARI', 'BASANT FLOORING PVTLTD', 'BATH DÉCOR', 'BATH JEWELS', 'BHOOMI ROAD CONTRACTOR', 'BRIGHT LIGHT', 'CERA HUB', 'D & K WATER TANK CLEANER', 'DARSHIL FABMET PVTLTD', 'DHANJI PANGAM', 'EARTHSTONE INFRA PVTLTD', 'ELLAPPA BABULAL PAWAR', 'EMMACE CORPORATION', 'GAUD WATER PUMP SERVICE', 'GAYATRI GLASS', 'GOEL POWER ENGINEERS', 'HALLMARK HUDRO PROJECTS', 'HELARAM MANDAL', 'HIMALAYA TRADERS', 'HKS FLOORING LLP', 'INDIA WATER PUMP', 'JATIN CORPORATION', 'JESUS ENTERPRISE', 'JESUS ENTERPRISES', 'KALIMI ENTERPRISES', 'KAY TEE INTERNATIONAL', 'LABDHI TRADING CO', 'LABHDI TRADER', 'LEE TILES CERAMICS', 'LIME LIGHT', 'M D ENTERPRISES', 'MALACH CONSTRUCTION', 'MALLAPPA PUJARI', 'MAP WORLDWIDE SERVICES', 'MAYUR CONSTRUCTION', 'MD ENTERPRISE', 'MIDAS CONSTRUCTION', 'MIRA MACHINE TOOLS', 'MK ENTERPRISE', 'MK ENTERPRISES', 'NAIK ENVIRONMENTAL ENGINEERS', 'NARAYAN WATER SUPPLIES', 'NATIONAL ELECTRICAL WORLD', 'NATIONAL PLY LAMINATE', "NEAT 'N' CLEAN", 'NJ ENTERPRISES', 'OM SAI APPLIANCE', 'OM SAI APPLIANCES', 'ORANGE MACHINETECH PVT LTD', 'PARTH HIGH RISE SAFETY', 'PIE ENTERPRISES', 'PIYALI DAS MANDAL', 'PLUMTECH ENGINEERING', 'POLLUCON ENVIRO ENGINEERS', 'POLLUCON ENVIRONMENT ENGINEERS PVTLTD', 'PRIYAL FABRICATION', 'R TECH ENGINEERING', 'RAKHI MANDAL', 'RAM PRAVESH GUPTA', 'RAYEES IMPEX PVTLTD', 'RISHAB STONE', 'S M ENTERPRISE', 'SAGAR CONSTRUCTION LIFTS', 'SAMARTH CONSTRUCTION', 'SAMBHAV TRADERS', 'SAMRAT ENTERPRISES', 'SHAMBHAV TRADERS', 'SHAMSUDDIN SHAIKH', 'SHREEJI ENTERPRISES', 'SHRI SATGURU KRIPA EARTH MOVERS', 'SIDDHI ENTERPRISE', 'SJASSOCIATES', 'SK HARDWARE', 'SM ENTERPRISES', 'SNMANE ROAD CONTRACTOR', 'SOLANKI TRADER', 'SOLANKI TRADERS', 'SOLGEN GREENTECH LLP', 'SPLENDID ENTERPRISES', 'TAG CONSTRUCTION AND GREEN SOLUTION', 'TK ENTERPRISES', 'TOSHIBA JOHNSON ELEVATORS INDIA PVT LTD', 'UN COPORATION', 'VIVAN WATER & ENVIRO SOLUTION', 'WELCOME COLOURS AND CHEMICALS', 'YASH ENTERPRISE'];
    ...
    async function fetchProject() {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          setEndDate(data.data.end_date ? data.data.end_date.split("T")[0] : "");
  ...
  function addLabourType() {
    setMetadata(prev => ({ ...prev, labour_type: [...prev.labour_type, ""] }));
  }
  ...
  function handleAddLabourType(e) {
    const val = e.target.value.trim();
    if (val && !metadata.labour_type.includes(val) && labourDict.includes(val)) {
      setMetadata(prev => ({ ...prev, labour_type: [...prev.labour_type, val] }));
    }
    e.target.value = "";
  }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 px-6 py-8 md:px-16 lg:px-28">
      <h1 className="text-3xl font-extrabold mb-8 text-white text-center">Daily Progress Report Editor</h1>

      {/* Project Details */}
      <div className="bg-gray-800 rounded-xl p-6 mb-8 border border-gray-700 shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-[#BBDEFB]">Project Details</h2>
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="projectName" className="block text-gray-300 mb-1 font-semibold">
                Project Name
              </label>
              <input
                id="projectName"
            ...
            <div>
              <label htmlFor="employer" className="block text-gray-300 mb-1 font-semibold">
                Employer
              </label>
              <input
                id="employer"
            ...
            <div>
              <label htmlFor="projectCode" className="block text-gray-300 mb-1 font-semibold">
                Project Code
              </label>
              <input
                id="projectCode"
            ...
            <div>
              <label htmlFor="location" className="block text-gray-300 mb-1 font-semibold">
                Location
              </label>
              <input
                id="location"
            ...
            <div>
              <label htmlFor="startDate" className="block text-gray-300 mb-1 font-semibold">
                Start Date
              </label>
              <input
                id="startDate"
            ...
            <div>
              <label htmlFor="endDate" className="block text-gray-300 mb-1 font-semibold">
                End Date
              </label>
              <input
                id="endDate"
            ...
            <div className="md:col-span-2">
              <label htmlFor="projectDescription" className="block text-gray-300 mb-1 font-semibold">
                Project Description
              </label>
              <textarea
          ...
          <button
            ...
            className={`mt-6 w-full px-5 py-2 rounded font-semibold ${
              savingDetails ? "bg-blue-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
            }`}
          >
      ...
      <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg">
        <h2 className="text-xl font-semibold mb-4 text-[#BBDEFB]">Agency & Labour Type Metadata</h2>

        <div className="mb-4 flex flex-wrap gap-4 items-end">
          <div>
            <label htmlFor="agencyInput" className="block mb-1 font-semibold text-gray-300">Add Agency</label>
            <input
              id="agencyInput"
              list="agency-list"
              placeholder="Select or type agency"
              onBlur={handleAddAgency}
              onKeyDown={e => { if(e.key === "Enter") { e.preventDefault(); handleAddAgency(e); } }}
          ...
          <div className="flex flex-row items-end gap-2">
            <div>
              <label htmlFor="labourInput" className="block mb-1 font-semibold text-gray-300">Add Labour Type</label>
              <input
                id="labourInput"
                list="labour-list"
                placeholder="Select or type labour type"
                onBlur={handleAddLabourType}
                onKeyDown={e => { if(e.key === "Enter") { e.preventDefault(); handleAddLabourType(e); } }}
                className="bg-gray-700 text-white rounded px-3 py-1 w-64 outline-none"
              />
          ...
          <table className="min-w-full border border-gray-600 table-auto">
            ...
            <tbody>
              {metadata.agency.length === 0 ? (
                <tr>
                  <td colSpan={metadata.labour_type.length + 1} className="text-center py-4 text-gray-400 italic">
                ...
                metadata.agency.map((agency, i) => (
                  ...
                  >
                    ...
                    {metadata.labour_type.map((_, j) => (
                      <td key={j} className="border border-gray-600 p-2 bg-gray-800" />
                    ))}
                  </tr>
                ))
              )}
        ...
        <button
          ...
          className={`mt-4 px-6 py-2 rounded font-semibold ${
            savingMetadata ? "bg-blue-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
          }`}
        >
