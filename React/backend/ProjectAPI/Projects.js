...
//get call to fetch teh user roles for teh project
router.get("/getuserroles/:id", authenticateJWT, async (req, res) => {
  try {
    ...
    const user_roles = await DB.getUsersInvolvedInProject(id);
    ...
    console.error("❌ API error in getuserroles:", error.message);
...
router.post("/updateProject/:project_id", authenticateJWT, async (req, res) => {
  try {
    ...
    if (updated === 0) {
      return res
        .status(404)
        .json({ ok: false, message: "No project fields updated" });
    }
...
router.post("/updateMetadata", authenticateJWT, async (req, res) => {
  try {
    ...
    if (!project_id || !metadata) {
      return res
        .status(400)
        .json({ message: "Missing project_id or metadata" });
    }
