...
function fetchAndDisplayDPR(dprId = 17) {
    fetch(`http://34.47.131.237:3000/report/getDPR/${dprId}`)
        ...
        .then(({ data }) => {
            ...
            for (let i = 0; i < labour.agency.length; i++) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${labour.helper[i] + labour.staff[i] + labour.mason[i] + labour.carp[i] + labour.fitter[i] + labour.electrical[i] + labour.painter[i] + labour.gypsum[i] + labour.plumber[i]}</td>
            ...
            // 4. Today Progress
            populateTable(data.today_prog.progress.map((p, i) => [p, data.today_prog.qty[i]]), todayTable, true);

            // 5. Tomorrow Plan
            populateTable(data.tomorrow_plan.plan.map((p, i) => [p, data.tomorrow_plan.qty[i]]), tomorrowTable, true);
        ...
        .catch(err => {
            ...
            // Set current date
            // Load DPR data
            fetchAndDisplayDPR(17); // Replace 17 with dynamic ID if needed
